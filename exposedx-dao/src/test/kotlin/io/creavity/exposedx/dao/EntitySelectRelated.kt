...
class EntitySelectRelated {
    ...
    fun assertNQuerys(n: Int, transaction: () -> Unit) {
        ...
        transaction {
            transaction()
            verify(exactly = n) { this@EntitySelectRelated.connection.prepareStatement(any(), any<Int>()) }
        }
    }

    @Test
    fun `Test select related querys`() {
    ...
    fun `Test count of query of prefetch related with one to one `() {
        ...
        assertNQuerys(1) {
            companies = Company.objects.selectRelated(Company.manager.country).all().toList()
        }
    ...
    @Test
    fun `Test count of query of select related with one to optional `() {
        val peru = Country.new { name="Peru" }
        val argentina = Country.new { name="Argentina" }
        ...
        assertNQuerys(1) {
            companies = Company.objects.selectRelated(Company.contactInfo.country).all().toList()
        }
    ...
    @Test
    fun `Test count of query of prefetch select of inverse of manyToOne`() {
        val c1 = Country.new  { name="Country" }
        val r = Region.new { name = "Region"; country = c1}
        val r2 = Region.new { name = "Region 2"; country = c1}
        School.new { name = "School"; region = r }
        ...
        assertNQuerys(1) {
            countries = Country.objects.selectRelated(Country.regions).all().toList()
        }
    ...
    fun `Test count of query of prefetch related inverse with one to one`() {
        ...
        assertNQuerys(1) {
            managers = Manager.objects.selectRelated(Manager.company).all().toList()
        }
